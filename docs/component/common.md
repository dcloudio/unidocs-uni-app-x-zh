...
<!-- CUSTOMTYPEJSON.global-properties.example -->
...
<!-- CUSTOMTYPEJSON.touch-events.example -->
...
<!-- CUSTOMTYPEJSON.transition-events.example -->
...
<!-- CUSTOMTYPEJSON.touch-events-case.example -->

#### 阻止默认行为

在事件回调中，可以通过调用`event.preventDefault`方法阻止默认行为。`event.preventDefault`仅处理默认行为，事件冒泡不会被阻止。

<!-- CUSTOMTYPEJSON.touch-events-preventDefault-click.example -->
