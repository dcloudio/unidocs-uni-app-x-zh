...
而原生应用中的页面默认是不可滚动的，需要滚动时需显式的在template中写scroll-view、list-view、waterflow等滚动容器的代码。

uni-app中，vue页面默认可滚动，而nvue为了多端一致性默认给页面套了一个scroll-view。

uni-app x的页面是一个原生页面，页面默认到底能不能滚动，即要不要隐式的给页面套一个scroll-view，是个性能和多端一致性的平衡问题。

### vdom模式app平台的页面默认不可滚动
在uni-app x的vdom模式，出于性能考虑，app平台默认没有隐式给页面套scroll-view，所以新建页面时，选择可滚动页面会出现下面的代码
-->

<!-- 
### 蒸汽模式页面默认可滚动
uni-app x的蒸汽模式，因为已经做到比原生渲染更快，不怕套一层scroll-view影响页面性能，所以拉平了多端一致性。

从5.11起，App平台蒸汽模式的页面，在符合条件的情况下，会由编译器自动套一层scroll-view。即，蒸汽模式的页面默认也可以滚动了。

- 并不是所有的情况都会自动套，如果是如下情况，就不会套一层scroll-view：

1. pages.json的页面Style中配置`disableScroll:true`禁用页面滚动，则不会套scroll-view。此配置项不影响页面中自行写scroll-view。
2. 页面是单根节点，且根节点已经是滚动容器（scroll-view、list-view、waterflow），不会多套一层scroll-view，但会把页面上设置的滚动相关属性，合并到根节点上。
3. 页面是多根节点，且有一个根节点是滚动容器（scroll-view、list-view、waterflow），不会多套一层scroll-view。也不会合并页面上设置的滚动相关属性。这种情况适用于自定义导航栏或tabbar场景

编译器对于页面根节点的滚动容器（scroll-view、list-view、waterflow）的分析是静态的，
不会判断滚动容器上是否存在v-if，
以及如果根节点的滚动容器被包装成其他名称的自定义组件，那么编译器也无法识别。

#### 页面可滚动的注意事项

页面的概念，在多端尤其是小程序端，仍然存在一些难以拉齐的表现。
小程序平台的页面的参数不丰富、且无法动态修改。
再加上现代浏览器的区域滚动性能问题已经解决，
所以如果追求严格的多端一致性，可以在pages.json中disableScroll禁用页面滚动，然后全平台使用scroll-view自行控制滚动行为，是更可控的做法。

当页面Style中开启了页面下拉刷新，即enablePullDownRefresh为true时，且`disableScroll:true`，此时页面仍无法滚动，但是否同时还支持下拉刷新，各平台有差异。此时微信小程序在Android、iOS、鸿蒙上的表现也不同。
