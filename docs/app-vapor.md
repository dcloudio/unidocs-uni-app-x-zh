...
App平台的蒸汽模式下，视图层不再以虚拟DOM作为主要运行时结构，而是把`template`和`style`编译为更接近底层渲染系统的产物。

根据编译目标不同，App平台的视图层产物分为字节码和机器码两种模式。

### 字节码

字节码模式，是把`template`和`style`编译为高效的二进制字节码，再由运行时解释执行。
