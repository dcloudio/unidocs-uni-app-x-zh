...
1. 将以下源文件(`SDK/ExtApiSrc 目录下`)、依赖库(`SDK/Libs 目录下`)添加到`插件工程`中，并将依赖库均设置为`Do Not Embed`
    ...
    |---|---|---|---|
    | UTSOC.h、UTSOC.mm、uts-config.json | DCloudUniappRuntime.xcframework、DCloudUTSExtAPI.xcframework、DCloudUTSFoundation.xcframework | DCloudUTSConfig.h、DCloudUTSConfig.m、UTSCPP.h、UTSCPP.mm、uts-config.json | DCUniBase.framework、DCloudUTSFoundation.framework  |
