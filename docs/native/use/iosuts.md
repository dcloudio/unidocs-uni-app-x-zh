...
1. 将以下源文件(`SDK/ExtApiSrc 目录下`)、依赖库(`SDK/Libs 目录下`)添加到`插件工程`中，并将依赖库均设置为`Do Not Embed`
    | 源码文件&资源文件 | uniapp-x项目依赖库| uniapp项目依赖库 |
    |---|---|---|
    | UTSOC.h、UTSOC.mm、uts-config.json | DCloudUniappRuntime.xcframework、DCloudUTSExtAPI.xcframework、DCloudUTSFoundation.xcframework | DCUniBase.framework、DCloudUTSFoundation.framework |
