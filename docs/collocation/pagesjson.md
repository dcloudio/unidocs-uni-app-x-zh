...
- 横屏
	...
	* iOS：在iPhone手机上默认竖屏，在ipad上默认自适应，ipad旋转为横屏后uni-app x应用也会横屏。可以在项目的 Info.plist 中配置应用可支持的横竖屏列表，详见[应用可支持横竖屏列表配置](https://uniapp.dcloud.net.cn/tutorial/app-nativeresource-ios.html#orientation)，配置后应用会限定在可支持的横竖屏列表中（如配置应用仅支持横屏，则应用只能显示为横屏）。uni-app x的页面和基础组件都支持自适应宽屏界面
- 状态栏
