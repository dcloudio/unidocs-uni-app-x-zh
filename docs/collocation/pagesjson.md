...
- 横屏
	...
	* iOS：在iPhone手机上目前只能竖屏，在ipad上只能自适应，ipad旋转为横屏后uni-app x应用也会横屏。可以在项目的 Info.plist 中配置应用支持的横竖屏，详见[应用横竖屏配置](https://uniapp.dcloud.net.cn/tutorial/app-nativeresource-ios.html#orientation)。uni-app x的页面和基础组件都支持自适应宽屏界面
- 状态栏
