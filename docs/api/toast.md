...
- toast 行为描述
  - 在 Android、iOS 端，toast行为与页面绑定，当打开新页面（包括 dialogPage）时，原页面弹出的 toast 会被遮挡(Android端 position字段生效时除外)
  - 在 HarmonyOS 端 设置 position 为 bottom 时，是和 App window 绑定的，当打开新页面时，原页面弹出的 toast 不会被遮挡
